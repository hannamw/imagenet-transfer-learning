...
data = {"train-val":{"images": [], "labels": []}, "test":{"images":[], "labels":[]}}
...
data["train-val"]["images"], data["train-val"]["labels"] = sklearn.utils.shuffle(data["train-val"]["images"], data["train-val"]["labels"])

###################

base_model = keras.applications.nasnet.NASNetMobile(input_shape = (224,224,3),include_top = False, weights ='imagenet', pooling='avg')
...
model.fit(data["train-val"]["images"], data["train-val"]["labels"], epochs = 15, validation_split = 0.2, batch_size = 20)
